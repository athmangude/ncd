...
export function CareFundCard() {
  const { showBalance, toggleBalance } = usePersistentBalance("careFundBalanceVisible")
    ...
    >
         ...
         <div className="relative z-10 flex flex-col flex-grow justify-between">
           ...
           <div className="flex flex-col mt-auto gap-[clamp(0.125rem,0.1rem+0.125vw,0.25rem)]">
             ...
             <p className="text-white/90 font-medium text-[clamp(12px,11px+0.5vw,14px)]">Cashback balance</p>
             <div className="flex items-center gap-3">
               <h2 className="font-bold tracking-tight leading-none text-[clamp(24px,22px+1vw,30px)]">
                 {showBalance ? formatMoney(rawBalance ?? 0, currencyCode, true) : "KES ****"}
               </h2>
               <button onClick={toggleBalance} className="text-white/80 hover:text-white transition-colors p-1 rounded-full hover:bg-white/10">
