...
const HelpItemCard: FC<HelpItem> = ({ label, icon, onClick, href }) => {
  ...
  return (
    <div
      className="flex justify-between bg-white p-4 items-center cursor-pointer hover:bg-neutral-100 rounded-lg transition border border-neutral-100"
      onClick={handleClick}
    >
      <div className="flex gap-3 items-center">
        {icon && <span className="text-neutral-500">{icon}</span>}
        <p className="text-sm sm:text-base font-medium text-neutral-700">{label}</p>
      </div>
      <ChevronRight className="h-5 w-5 text-neutral-400 flex-shrink-0 self-center" />
    </div>
  )
}
  ...
  const invoiceProblems: HelpItem[] = [
    {
      ...
      // Placeholder action
      onClick: () => console.log("Details mismatch clicked"), 
    },
    {
      label: "How to find downloaded files on your phone",
      icon: <HelpCircle className="h-5 w-5" />,
      // Placeholder action
      onClick: () => console.log("How to find downloaded files on your phone clicked"),
    },
    {
      label: "Invoice is not valid?",
      icon: <HelpCircle className="h-5 w-5" />,
      // Placeholder action
      onClick: () => console.log("Invoice invalid clicked"),
    },
  ]
    ...
    <PatientPageWrapper title="Help & Support">
      <div className="flex flex-col gap-6 p-1">
        <div className="flex flex-col gap-2">
          <h1 className="text-xl font-bold text-black">We're here to help.</h1>
        ...
        <div className="flex flex-col gap-4">
          <h2 className="text-sm text-neutral-500 font-medium">
        ...
        <div className="flex flex-col gap-4">
          ...
          <Card className="bg-white p-2 rounded-xl border-neutral-200">
