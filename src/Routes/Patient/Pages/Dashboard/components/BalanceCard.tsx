...
export function BalanceCard({  loanStats, isLocked = false, onUpgrade, isLoading }: BalanceCardProps) {
  const { showBalance, toggleBalance } = usePersistentBalance("paymentsBalanceVisible")
  ...
  if (isLocked) {
    return (
      <div
        onClick={onUpgrade}
        className="relative w-full aspect-[1.586] rounded-[clamp(1.5rem,1.35rem+0.75vw,2rem)] overflow-hidden shadow-xl shadow-neutral-300 transition-all duration-300 hover:shadow-2xl hover:shadow-neutral-400/50 cursor-pointer hover:opacity-95"
      >
        {/* Muted grey base */}
        <div
          aria-hidden
        ...
        <div className="relative z-10 flex flex-col h-full justify-between p-[clamp(1.25rem,1.15rem+0.5vw,1.5rem)] text-white drop-shadow-md">
          ...
          <div className="flex flex-col mt-auto gap-[clamp(0.125rem,0.1rem+0.125vw,0.25rem)]">
            ...
            <p className="text-white font-semibold text-[clamp(12px,11px+0.5vw,14px)]">Available balance</p>
            ...
            <p className="text-white/90 font-medium text-[clamp(12px,11px+0.5vw,14px)] mt-[clamp(0.125rem,0.1rem+0.125vw,0.25rem)]">
              Including {showBalance ? formatMoney(outstandingAmount, currencyCode) : "KES ****"} loan
            </p>
          </div>
        </div>
  ...
  return (
    <div className="relative w-full aspect-[1.586] rounded-[clamp(1.5rem,1.35rem+0.75vw,2rem)] p-[clamp(1.25rem,1.15rem+0.5vw,1.5rem)] text-white overflow-hidden shadow-xl shadow-purple-200 bg-cover bg-center bg-no-repeat transition-all duration-300 hover:shadow-2xl hover:shadow-purple-300/50 bg-purple-600 flex flex-col"
         ...
         <div className="relative z-10 flex flex-col flex-grow justify-between">
           <div className="flex justify-between items-start">
            ...
            <div className="origin-top-right" style={{ transform: 'scale(calc(0.9 + 0.1 * (100vw - 320px) / 520))' }}>
             ...
             <p className="text-purple-100 font-medium text-[clamp(12px,11px+0.5vw,14px)]">Available Balance</p>
             <div className="flex items-center gap-3">
               <h2 className="font-bold tracking-tight leading-none text-[clamp(24px,22px+1vw,30px)]">
                 {showBalance ? formatMoney(remainingCreditLimit, currencyCode) : "KES ****"}
               </h2>
               <button onClick={toggleBalance} className="text-purple-200 hover:text-white transition-colors p-1 rounded-full hover:bg-white/10">
             ...
             <p className="text-purple-200 text-[clamp(12px,11px+0.5vw,14px)] mt-[clamp(0.125rem,0.1rem+0.125vw,0.25rem)]">
               Outstanding Loan : {formatMoney(outstandingAmount, currencyCode)}
             </p>
           </div>
         </div>
    </div>
