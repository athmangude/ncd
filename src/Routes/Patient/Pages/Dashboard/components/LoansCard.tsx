...
export function LoansCard({ loanStats, isLocked = false, onUpgrade, isLoading }: LoansCardProps) {
  const { showBalance, toggleBalance } = usePersistentBalance("loansBalanceVisible")
      ...
      <div className="flex flex-col gap-4" onClick={onUpgrade}>
        <div className="relative w-full aspect-[1.586] rounded-[clamp(1.5rem,1.35rem+0.75vw,2rem)] overflow-hidden shadow-xl shadow-neutral-300 transition-all duration-300 hover:shadow-2xl hover:shadow-neutral-400/50 cursor-pointer hover:opacity-95">
          ...
          <div className="relative z-10 flex flex-col h-full justify-between p-[clamp(1.25rem,1.15rem+0.5vw,1.5rem)] text-white drop-shadow-md">
            <div className="flex justify-between items-start">
              ...
              <div className="origin-top-right" style={{ transform: 'scale(calc(0.9 + 0.1 * (100vw - 320px) / 520))' }}>
            ...
            <div className="flex flex-col mt-auto gap-[clamp(0.125rem,0.1rem+0.125vw,0.25rem)]">
              <div className="mb-1 opacity-90">
                <ChipIcon style={{ width: 'clamp(36px, 34px + 1vw, 45px)', height: 'clamp(28px, 26.5px + 0.75vw, 35px)' }} />
              </div>
              <p className="text-white font-semibold text-[clamp(12px,11px+0.5vw,14px)]">Available to Borrow</p>
              <div className="flex items-center gap-3">
                <h2 className="font-bold tracking-tight leading-none text-white text-[clamp(24px,22px+1vw,30px)]">
                  {showBalance ? formatMoney(remainingCreditLimit, currencyCode) : "KES ****"}
                </h2>
                <button
                ...
                >
                  {showBalance
                    ? <EyeOff style={{ width: 'clamp(1rem, 0.95rem + 0.25vw, 1.25rem)', height: 'clamp(1rem, 0.95rem + 0.25vw, 1.25rem)' }} />
                    : <Eye style={{ width: 'clamp(1rem, 0.95rem + 0.25vw, 1.25rem)', height: 'clamp(1rem, 0.95rem + 0.25vw, 1.25rem)' }} />}
                </button>
              </div>
              <p className="text-white/90 font-medium text-[clamp(12px,11px+0.5vw,14px)] mt-[clamp(0.125rem,0.1rem+0.125vw,0.25rem)]">
                Total to repay: {showBalance ? formatMoney(outstandingAmount, currencyCode) : "KES ****"}
              </p>
            </div>
          </div>
        ...
        {outstandingAmount > 0 && (
          <div className="flex flex-col gap-3 px-1">
            <Progress value={progress} className="h-2 bg-purple-200" indicatorClassName="bg-[#A855F7]" />
            <div className="flex justify-between text-sm font-medium">
              <span className="text-neutral-600">Paid: <span className="text-neutral-900">{formatMoney(totalPaid, currencyCode)}</span></span>
              <span className="text-neutral-600">Due: <span className="text-neutral-900">{formatMoney(outstandingAmount, currencyCode)}</span></span>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    ...
    <div className="relative w-full aspect-[1.586] rounded-[clamp(1.5rem,1.35rem+0.75vw,2rem)] p-[clamp(1.25rem,1.15rem+0.5vw,1.5rem)] text-white overflow-hidden shadow-xl shadow-blue-200 bg-cover bg-center bg-no-repeat transition-all duration-300 hover:shadow-2xl hover:shadow-blue-300/50 bg-blue-600 flex flex-col"
    ...
    >
         ...
         <div className="relative z-10 flex flex-col flex-grow justify-between">
           <div className="flex justify-between items-start">
            ...
            <div className="origin-top-right" style={{ transform: 'scale(calc(0.9 + 0.1 * (100vw - 320px) / 520))' }}>
              <img src={logoIcon} alt="Jireh Logo" width="80" className="h-auto" />
            </div>
           </div>
             ...
             <p className="text-blue-100 font-medium text-[clamp(12px,11px+0.5vw,14px)]">Available to borrow</p>
             <div className="flex items-center gap-3">
               <h2 className="font-bold tracking-tight leading-none text-[clamp(24px,22px+1vw,30px)]">
                  {showBalance ? formatMoney(remainingCreditLimit, currencyCode) : "KES ****"}
               </h2>
               <button onClick={toggleBalance} className="text-blue-200 hover:text-white transition-colors p-1 rounded-full hover:bg-white/10">
                 {showBalance ? <EyeOff style={{ width: 'clamp(1rem, 0.95rem + 0.25vw, 1.25rem)', height: 'clamp(1rem, 0.95rem + 0.25vw, 1.25rem)' }} /> : <Eye style={{ width: 'clamp(1rem, 0.95rem + 0.25vw, 1.25rem)', height: 'clamp(1rem, 0.95rem + 0.25vw, 1.25rem)' }} />}
               </button>
             </div>
             <p className="text-blue-200 text-[clamp(12px,11px+0.5vw,14px)] mt-[clamp(0.125rem,0.1rem+0.125vw,0.25rem)]">
                Total  to repay {showBalance ? formatMoney(outstandingAmount, currencyCode) : "KES ****"}
             </p>
           </div>
         </div>
